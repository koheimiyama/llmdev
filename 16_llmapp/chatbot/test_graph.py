...
def test_define_tools():
    ...
    assert any(
        tool.name == "retrieve_company_rules" for tool in tools
...
def test_memory_clear_on_new_session(setup_memory):
    ...
    assert (
        cleared_messages is None or "channel_values" not in cleared_messages
    ), "メモリがクリアされていません。"
...
def test_get_messages_list(setup_memory):
    ...
    assert any(
        msg["class"] == "user-message" for msg in messages
    ), "メッセージリストにユーザーのメッセージが含まれている必要があります。"
    assert any(
        msg["class"] == "bot-message" for msg in messages
